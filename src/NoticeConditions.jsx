...
import {ApplicantBanner,PersonalCriterion} from './ApplicantProfile.jsx';
...
export function NoticeConditions({notice,checkedIds=[],heading,onCheckedChange,applicantProfile,onEditProfile}){
 const infoId=useId(),[expandedFor,setExpandedFor]=useState(null),[savingCheck,setSavingCheck]=useState(false);
 ...
 useEffect(()=>{
  ...
  (async()=>{
   ...
   try{bundle=await snapshots({key,force:attempt>0})}catch{snapshotFailed=true}
   const stored=bundle?.summaries?.[key];
   let data=!notice.analysisOutdated&&!notice.analysisStale?.conditions&&validEvidenceSummary(stored,key)?stored:null;
   if(cancelled)return;
 ...
 return <div className="notice-evidence" aria-busy={current.status==='loading'||current.status==='refreshing'}>
  ...
  {onEditProfile&&<ApplicantBanner profile={applicantProfile} onEdit={onEditProfile}/>}
  ...
  <div className="evidence-criteria">{displayItems.map(c=>{
   ...
   return <article className="evidence-criterion" key={c.id}>
    <div className="evidence-heading"><h3>{c.id==='income'?'소득·자산 기준':c.title}</h3>{onCheckedChange?<label className="evidence-check"><input type="checkbox" checked={read} disabled={savingCheck} aria-label={`${c.id==='income'?'소득·자산 기준':c.title} 확인했어요`} onChange={async e=>{const checked=e.target.checked;setSavingCheck(true);try{await onCheckedChange(c.id,checked)}finally{setSavingCheck(false)}}}/><span>확인했어요</span></label>:<span className={'evidence-read-state '+(read?'is-read':'')}>{read?'내가 확인함':'아직 확인 안 함'}</span>}</div>
    <PersonalCriterion profile={applicantProfile} notice={notice} criterion={c.id} rows={rows} incomeGuide={guide}/>
