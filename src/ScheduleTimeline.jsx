...
export function ScheduleTimeline({notice}){
 const key=evidenceNoticeKey(notice?.url),[loaded,setLoaded]=useState(null),[failed,setFailed]=useState(false),[selection,setSelection]=useState({key:null,index:0});
 useEffect(()=>{let cancelled=false;setFailed(false);loadSchedules().then(bundle=>{if(!cancelled)setLoaded(bundle)}).catch(()=>{if(!cancelled)setFailed(true)});return()=>{cancelled=true}},[key]);
 ...
 return <section className="application-timeline" aria-labelledby="application-timeline-title">
  ...
  {failed&&<p className="timeline-note" role="status">상세 일정을 불러오지 못했어요. 목록에서 확인한 날짜만 표시합니다.</p>}
