...
import {ApplicantBanner} from './ApplicantProfile.jsx';
import {AccountControls} from './AccountPage.jsx';
...
export function ProfileSettings({name,region,onSave,onBack,account,applicantProfile,onApplicantProfile}){
 const [saving,setSaving]=useState(false);
 const options=regions.includes(region)?regions:[region,...regions].filter(Boolean);
 return <section className="settings-panel profile-settings"><SettingsBack onBack={onBack}/><h2 tabIndex={-1} data-settings-focus>내 정보 설정</h2>{account.user&&<ApplicantBanner profile={applicantProfile} onEdit={onApplicantProfile}/>}<AccountControls {...account}>{account.user&&<form className="profile-settings-form" onSubmit={async e=>{e.preventDefault();const data=new FormData(e.currentTarget),name=String(data.get('name')||'').trim();if(!name){e.currentTarget.elements.name.setCustomValidity('이름을 입력해주세요.');e.currentTarget.reportValidity();return}setSaving(true);try{await onSave({name,region:data.get('region')})}finally{setSaving(false)}}}><label className="field-label">이름<input name="name" defaultValue={name} maxLength={16} required autoComplete="given-name" onInput={e=>e.currentTarget.setCustomValidity('')}/></label><label className="field-label">관심 지역<select name="region" defaultValue={region}>{options.map(value=><option key={value}>{value}</option>)}</select></label><button className="primary" type="submit" disabled={saving||account.busy}>{saving?'저장 중…':'저장하기'}</button></form>}</AccountControls></section>
}
